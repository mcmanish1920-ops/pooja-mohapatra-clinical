...
const saved=(()=>{try{return JSON.parse(localStorage.getItem('pujaSiteConfigV14')||localStorage.getItem('pujaSiteConfigV13')||'null')}catch(e){return null}})();
...
function render(){
 ...
 const heroSrc=sectionImages.hero||c.hero?.image||'assets/hero.jpg';
 const hero=$('#heroImage');if(hero)hero.src=heroSrc;
 [['about','about'],['services','services'],['approach','approach'],['journey','journey'],['practice','practice'],['reviews','reviews'],['faq','faq'],['contact','contact'],['cta','cta']].forEach(([id,key])=>{
   const src=sectionImages[key]; const section=$('#'+id); if(section&&src)section.style.setProperty('--section-image',`url("${src}")`);
   const img=$('#'+id+'Image'); if(img&&src){img.src=src;img.loading='lazy';img.decoding='async';}
 });
 ...
 fill('servicesGrid',(c.services||[]).map((x,i)=>`<article class="service-card reveal"><div class="service-icon">${['◉','♧','♡','▣'][i%4]}</div><h3>${esc(x.title)}</h3><p>${esc(x.text)}</p><span class="arrow">→</span></article>`).join(''));
...
(async()=>{try{
 const db=await new Promise((resolve,reject)=>{const r=indexedDB.open('pujaSiteCustomizerV13',1);r.onupgradeneeded=()=>{if(!r.result.objectStoreNames.contains('images'))r.result.createObjectStore('images',{keyPath:'id'});};r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error)});
